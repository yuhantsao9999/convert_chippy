...
Blockly.defineBlocksWithJsonArray([// BEGIN JSON EXTRACT
		...
		// Block for boolean data type: false and true.
        {
            "type": "logic_false",
            "message0": "%1",
            "args0": [{
        ...
        // Block for do then return
        {
            "type": "controls_do_then_return",
            "message0": "do %1 return %2",
            "args0": [{
                    "type": "input_statement",
                    "name": "STM"
                }, {
...
Blockly.Blocks['logic_or'] = {
    init: function () {
        ...
        .setCheck("Boolean")
        .appendField(new Blockly.FieldDropdown([["or", "OR"], ["and", "AND"]]), "OP");
    ...
    decompose: function (workspace) {
        var containerBlock = workspace.newBlock('logic_or_container');
    ...
    compose: function (containerBlock) {
        ...
        for (var i = 0; i < this.itemCount_; i++) {
            var connection;
            if (i == 0) {
                connection = this.getInput('A').connection.targetConnection;
            } else if (i == 1) {
                connection = this.getInput('B').connection.targetConnection;
            } else {
                connection = this.getInput('BOOL' + i).connection.targetConnection;
            }
        ...
        for (var i = 0; i < this.itemCount_; i++) {
            if (i == 0) {
                Blockly.Mutator.reconnect(connections[i], this, 'A');
            } else if (i == 1) {
                Blockly.Mutator.reconnect(connections[i], this, 'B');
            } else {
                Blockly.Mutator.reconnect(connections[i], this, 'BOOL' + i);
            }
        }
    },
    saveConnections: function (containerBlock) {
        ...
        while (itemBlock) {
            var input;
            if (i == 0) {
                input = this.getInput('A');
            } else if (i == 1) {
                input = this.getInput('B');
            } else {
                input = this.getInput('BOOL' + i);
            }
    ...
    updateShape_: function () {
        for (var i = 0; i < this.itemCount_; i++) {
            if (i == 0) {
                if (!this.getInput('A')) {
                    var input = this.appendValueInput('A').setCheck("Boolean");
                }
            } else if (i == 1) {
                if (!this.getInput('B')) {
                    var input = this.appendValueInput('B').setCheck("Boolean");
                    input.appendField("or");
                }
            } else {
                if (!this.getInput('BOOL' + i)) {
                    var input = this.appendValueInput('BOOL' + i).setCheck("Boolean");
                    input.appendField("or");
                }
            }
        }
        // Remove deleted inputs.
        while (this.getInput('BOOL' + i)) {
