...
Blockly.Python['logic_false'] = function (block) {
    // Boolean values true and false.
    var code = (block.getFieldValue('BOOL') == 'TRUE') ? 'True' : 'False';
    return [code, Blockly.Python.ORDER_ATOMIC];
};
...
Blockly.Python['controls_do_then_return'] = function (block) {
    // Do then return
    var statements = Blockly.Python.statementToCode(block, 'STM');
    var value = Blockly.Python.valueToCode(block, 'VALUE', Blockly.Python.ORDER_ATOMIC);
    var code = value + ' = ' + statements + '\n'; ;
...
Blockly.Python['logic_or'] = function (block) {
    ...
    for (var i = 0; i < block.itemCount_; i++) {
        var argument;
        if (i == 0) {
            argument = Blockly.Python.valueToCode(block, 'A',
                    Blockly.Python.ORDER_LOGICAL_OR);
            if (!argument)
                argument = 'False';
            code += argument;
        } else if (i == 1) {
			argument = Blockly.Python.valueToCode(block, 'B',
                Blockly.Python.ORDER_LOGICAL_OR);
			if (!argument)
                argument = 'False';
            code += " or " + argument;
        } else {
			argument = Blockly.Python.valueToCode(block, 'BOOL' + i,
                Blockly.Python.ORDER_LOGICAL_OR);
            if (!argument)
                argument = 'False';
            code += " or " + argument;
